...
-- КРАСИВОЕ СТАРТОВОЕ УВЕДОМЛЕНИЕ О ЗАГРУЗКЕ
game:GetService("StarterGui"):SetCore("SendNotification", {
    Title = "HUB ЗАГРУЖЕН",
    Text = "Успешное подключение к UN_Bypass.lua!",
    Duration = 5
})

-- СОЗДАНИЕ ГЛАВНОГО МЕНЮ (GUI)
local screenGui = Instance.new("ScreenGui")
screenGui.Name = "VillagePremiumHub"
...
titleLabel.Text = "VILLAGE HUB v2.0"
...
local tpBtn = createButton("Авто-ТП: ВЫКЛ", UDim2.new(0, 10, 0, 55), Color3.fromRGB(180, 50, 50), function()
...
-- 2. КНОПКА: ПОЛЕТ (FLY)
local flyBtn = createButton("Полет: ВЫКЛ", UDim2.new(0, 10, 0, 105), Color3.fromRGB(180, 50, 50), function()
    ...
    if flyEnabled then
        ...
        task.spawn(function()
            while flyEnabled and root and bv.Parent do
                bv.Velocity = player:GetMouse().Hit.LookVector * 100 -- Летит туда, куда направлена камера
                task.wait(0.01)
            end
            bv:Destroy()
        end)
...
-- 3. КНОПКА: ВЫСОКИЙ СУПЕР ПРЫЖОК (INFINITE JUMP)
local jumpBtn = createButton("Прыжки: ВЫКЛ", UDim2.new(0, 10, 0, 155), Color3.fromRGB(180, 50, 50), function()
